...
def getFile(file, jd):
    pdfText = pdfEctracter(file)
    filtered_pdf_words = fiterText(pdfText)
    filtered_jd_words = fiterText(jd)
    filtered_pdf_words = set(filtered_pdf_words)
    filtered_jd_words = set(filtered_jd_words)
    common_words = filtered_pdf_words.intersection(filtered_jd_words)
